...
const FoodCard = memo(({ data, hideFooter = false }) => {
  ...
  const isAvailable =
    data.stockStatus === "in_stock" || data.stockStatus === "limited";
  ...
  const discountedPrice = useMemo(() => {
    return hasOffer
      ...
      : data.price;
  }, [hasOffer, data.price, data.offerPercentage]);
  ...
  return (
    <div
      className={`group w-full max-w-[280px] rounded-2xl border border-gray-100 bg-white shadow-md overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col ${
        !canOrder ? "opacity-70 grayscale-[0.5]" : ""
      }`}
    >
      <div className="relative w-full h-[180px] overflow-hidden bg-gray-50">
        ...
        {hasOffer && (
          <div className="absolute bottom-3 left-3 bg-[#60b246] text-white px-3 py-1 rounded-md text-[11px] font-black shadow-lg z-10 flex flex-col items-center leading-none border border-white/20">
            ...
            <span className="text-[7px] opacity-90 uppercase tracking-tighter">
              Limited Deal
            </span>
          </div>
        )}
      ...
      {!hideFooter && (
        <div className="flex items-center justify-between mt-auto p-4 bg-gray-50/50">
          ...
          <div
            className={`flex items-center bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm transition-all duration-300 focus-within:ring-2 focus-within:ring-[#ff4d2d]/20 ${
              !canOrder
                ? "opacity-50 pointer-events-none"
                : "hover:border-[#ff4d2d]/30"
            }`}
          >
            <button
            ...
            <button
              className={`${
                cartItems.some((i) => i.id == data._id)
                  ? "bg-gray-900"
                  : "bg-[#ff4d2d]"
              } text-white px-3.5 py-2.5 hover:opacity-90 active:scale-95 transition-all shadow-inner`}
              onClick={() => {
                if (!canOrder) return;
                quantity > 0
                  ? dispatch(
