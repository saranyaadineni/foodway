...
function EditItem() {
  ...
  useEffect(() => {
    ...
    setHasOffer(currentItem?.hasOffer || false);
    setOfferPercentage(currentItem?.offerPercentage || 0);
    setFrontendImage(getImageUrl(currentItem?.image) || "");
  }, [currentItem]);
              ...
              <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="relative group">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-xl px-4 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-[#ff2b85] transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    onChange={(e) => setOfferPercentage(Math.max(0, Math.min(100, e.target.value)))}
                    value={offerPercentage}
                  />
                  <div className="absolute right-10 top-1/2 -translate-y-1/2 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      type="button"
                      onClick={() => setOfferPercentage(prev => Math.min(100, Number(prev) + 1))}
                      className="text-gray-400 hover:text-[#ff2b85] transition-colors -mb-1"
                    >
                    ...
                    <button 
                      type="button"
                      onClick={() => setOfferPercentage(prev => Math.max(0, Number(prev) - 1))}
                      className="text-gray-400 hover:text-[#ff2b85] transition-colors -mt-1"
                    >
                      <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[#ff2b85] font-black pointer-events-none select-none">%</span>
                </div>
